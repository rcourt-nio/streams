use crate::config::{SatelliteCategory, SatelliteConfig};
...
pub struct TelemetryOutput {
    ...
    pub communication_band: &'static str,
...
pub fn compute(
    ...
    // --- Attitude (pitch & roll, deg) ---
    // Nadir-pointing baseline with attitude-control jitter. Earth-observation
    // satellites can also have small commanded off-pointing for targeting.
    let (pitch_amp, roll_amp) = match config.category {
        SatelliteCategory::EarthObservation => (4.0, 4.0),
        SatelliteCategory::Science => (3.0, 3.0),
        _ => (1.5, 1.5),
    };
    let pitch_deg = pitch_amp
        * layered_noise(id, timestamp_secs, "pitch", &[(180.0, 0.6), (45.0, 0.4)]);
    let roll_deg = roll_amp
        * layered_noise(id, timestamp_secs, "roll", &[(180.0, 0.6), (45.0, 0.4)]);

    // --- Operational Status ---
    let maintenance_cycle = ((timestamp_secs / 86400.0).fract() * 86400.0) % 86400.0;
    let in_maintenance = maintenance_cycle > 82800.0; // Last 60 min of each day

    let operational_status = if battery_level_percent < 15.0 {
        "SafeMode"
    } else if in_maintenance {
        "Maintenance"
    } else if state.elevation_deg < 0.0 {
        "Standby"
    } else {
        "Active"
    };
    ...
    // --- Communication Band ---
    let communication_band = if state.elevation_deg > 30.0 {
        "Ka-band"
    } else if state.elevation_deg > 10.0 {
        "X-band"
    } else if state.elevation_deg > 5.0 {
        "S-band"
    } else {
        "UHF"
    };
    ...
    // --- Power Mode ---
    let power_mode = if !state.is_sunlit {
        "Eclipse"
    } else if battery_level_percent < 30.0 {
        "LowPower"
    } else if battery_level_percent < 90.0 {
        "Charging"
    } else {
        "Nominal"
    };
